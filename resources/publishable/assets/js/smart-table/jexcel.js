...
var jexcel = (function(el, options) {
    ...
    var defaults = {
        ...
        formulaFields: [],
        computationFields: [],
        computationToMultiple: [],
        computationGroupField: false,
    ...
    obj.prepareTable = function() {
        ...
        for (var i = 0; i < size; i++) {
            ...
            if (! obj.options.columns[i].field) {
                obj.options.columns[i].field = obj.options.fields[i] ? obj.options.fields[i] : '';
            }
    ...
    /**
     * Get the whole group table data
     *
     * @param integer row number
     * @return string value
     */

    obj.getGroupData = function(highlighted) {
        // Control vars
        var dataset = {};
        var px = 0;
        var py = 0;
        ...
        var fields =  obj.options.fields.filter(x => !Object.keys(groupFileds).includes(x));
        for (var j = 0; j < y; j++) {
            px = 0;

            if (obj.records[j][0].getAttribute("data-merged")){
                mergeCounter++;
                groupFileds[obj.options.groupRecords[mergeCounter]['field']] = obj.options.groupRecords[mergeCounter]['id'];
            ...
            for (var i = 0; i < x; i++) {
                ...
                if (! highlighted || obj.records[j][i].classList.contains('highlight')) {
                    ...
                    if (! dataset[py])
                        dataset[py] = {};
                    //-- check if data is formula return formula value --//
                    if (obj.options.data[j][i] !== undefined) {
                        if (String(obj.options.data[j][i]).match(/^\=/)) {
                            if (obj.rows[j].getElementsByTagName("td")[i+1].textContent.match(/NaN/))
                                dataset[py][fields[px]] = 0;
                            else
                                dataset[py][fields[px]] = obj.rows[j].getElementsByTagName("td")[i+1].textContent;
                        } else {
                            dataset[py][fields[px]] = (obj.options.data[j][i]!="") ?obj.options.data[j][i]:null;
                        }
                    }
                    px++;
                }
            ...
            isGroup = false;
            if (Object.keys(groupFileds).length > 0 && groupFileds.constructor === Object){
                var copy = Object.assign({}, groupFileds);
                //dataset[dataset[obj.options.rowIds[py]]].group = copy;
                dataset[py] = Object.assign({}, dataset[py], copy);
            }
            if (px > 0) {
                py++;
            }
    ...
    /**
     * Compute the whole group table data
     *
     * @param  string
     * @return object
     */

    obj.computeData = function() {
        let fields            =  obj.options.fields.filter(x => !Object.keys(groupFileds).includes(x));
        let computationFields =  obj.options.computationFields;
        let computationToMultiple = obj.options.computationToMultiple;
        let data              =  obj.getGroupData();
        let table_data        =  { 'data' :[]};
        let group_filed       = obj.options.computationGroupField;
        table_data.group      =  obj.options.columns[fields.indexOf(group_filed)];
        table_data.headers    =  ['Ընդհանուր քանակ'];

        computationFields.forEach(function(field) {
            table_data.headers.push(obj.options.colHeaders[fields.indexOf(field)]);
        });

        computationToMultiple.forEach(function(field) {
            table_data.headers.push(field.name);
        });
        for(let i of Object.keys(data)){
            if( !table_data.data[data[i][group_filed]])
                table_data.data[data[i][group_filed]] = {...{count:0}, ...computationFields.reduce((a,b)=>(a[b]=0,a),{})};
            table_data.data[data[i][group_filed]].count++;
            computationFields.forEach(function(field) {
                if (data[i][field]) table_data.data[data[i][group_filed]][field] += parseInt(data[i][field])?parseInt(data[i][field]):0;
            });
            computationToMultiple.forEach(function(field) {
                if (!table_data.data[data[i][group_filed]][field.column+'_'+field.id])
                    table_data.data[data[i][group_filed]][field.column+'_'+field.id] = 0;
                if (data[i][field.column]) table_data.data[data[i][group_filed]][field.column+'_'+field.id] += (parseInt(data[i][field.column]) && field.id == parseInt(data[i][field.column]))?parseInt(data[i][field.relationColumn]):0;
            });
        }
        return table_data;
    }
    ...
    obj.createCellHeader = function(colNumber) {
        ...
        var field = obj.options.columns[colNumber].field ? obj.options.columns[colNumber].field : obj.options.fields[colNumber];
        ...
        obj.headers[colNumber].setAttribute('class', field);
    ...
    obj.closeEditor = function(cell, save) {
        ...
        if (save == true) {
            ...
            var oldValue = obj.options.data[y][x];
            if (value && oldValue && value != oldValue && (oldValue.toString().match(/^\=/) || value.toString().match(/^\=/) ) ){
                var formulaFields =  obj.options.formulaFields.filter(x => !Object.keys(groupFileds).includes(x));
                var execFormulaPos = obj.options.data[y].length-1;
                var execFormula = obj.options.data[y][execFormulaPos];

                try {
                    execFormula = JSON.parse(execFormula);
                }
                catch (e) {
                    if (typeof execFormula != 'object')
                        execFormula = {};
                }
                if (oldValue.toString().match(/^\=/) && !value.toString().match(/^\=/)){
                    execFormula[formulaFields[x]] = true;
                }
                else if (!oldValue.toString().match(/^\=/) && value.toString().match(/^\=/)){
                    delete execFormula[formulaFields[x]];
                }
                obj.options.data[y][execFormulaPos] = execFormula;
            }
    ...
    obj.resetSelection = function(blur) {
        // Remove style
        obj.highlighted.forEach(function(element) {
            if (element.getAttribute("data-merged")) {
...
jexcel.methods.text = (function() {
    ...
    exports.MID = function(text, start, number) {
        ...
        return text.toString().length>0?parseInt(text.toString().substring(begin, end)):'';
    };
