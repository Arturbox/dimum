...
(function ($, window) {
  ...
  FreezeTable.prototype.buildColumnTable = function() {
    ...
    this.$container.on('resize.'+this.namespace, function() {
      ...
      var width = 8 + columnBorderWidth;
      for (var i = 1; i <= columnNum; i++) {
        // th/td detection
        var th = that.$table.find('tr:last-child').find('th:nth-child('+i+')').outerWidth();
        var addWidth = (th > 0) ? th : that.$table.find('tr:last-child').find('td:nth-child('+i+')').outerWidth();
        width += addWidth;
      }
